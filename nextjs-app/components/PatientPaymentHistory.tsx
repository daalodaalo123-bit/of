          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {entries.map((e, idx) => (
                  <div key={`${e.payment.id}-${e.transactionIndex ?? 'main'}-${idx}`} className="border border-gray-200 rounded-xl p-4 flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <button type="button" onClick={() => onDownloadReceipt(e.receiptPayment)} className="px-3 py-1.5 text-sm font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                        Receipt
                      </button>
                      {onEditPayment && e.transactionIndex == null && (
                        <button type="button" onClick={() => onEditPayment(e.payment)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-600" title="Edit">
