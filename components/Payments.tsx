  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      <div className="mb-3 sm:mb-4 lg:mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-lg sm:text-xl lg:text-4xl font-semibold text-white mb-1 sm:mb-2 tracking-tight">Payments Management</h1>
          <p className="text-gray-400 text-xs sm:text-sm lg:text-lg hidden sm:block">Track patient payments and balances.</p>
        </div>
        <button
      ...
      <div className="bg-[#1a1d27] rounded-xl sm:rounded-2xl shadow-sm border border-gray-700/50 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="hidden lg:table w-full min-w-[640px]">
            <thead className="bg-[#242732] border-b border-gray-700/50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">#</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Patient</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Phone</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Total (from registration)</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Amount Paid</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Balance</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider w-28">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-700/50">
              {groupedByPatient.map((g, i) => (
                <tr
                  key={g.patientId}
                  className="hover:bg-[#242732]/50 transition-colors cursor-pointer"
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    <span className="block text-xs text-gray-500 mt-0.5">Click to view payment history</span>
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-400 tabular-nums">{g.patientPhone || '-'}</td>
                  <td className="px-4 py-3 text-right text-gray-300 tabular-nums whitespace-nowrap">${g.totalFromRegistration.toFixed(2)}</td>
                  <td className="px-4 py-3 text-right font-medium text-green-400 tabular-nums whitespace-nowrap">${g.totalPaid.toFixed(2)}</td>
                  <td className="px-4 py-3 text-right font-medium tabular-nums whitespace-nowrap">{g.totalBalance > 0 ? <span className="text-amber-400">${g.totalBalance.toFixed(2)}</span> : <span className="text-gray-500">$0.00</span>}</td>
                  <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        onClick={() => setHistoryPatient({ id: g.patientId, name: g.patientName, phone: g.patientPhone })}
                        className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-700/50 text-gray-400"
                        title="View history"
                      >
            ...
            {groupedByPatient.length === 0 ? (
              ...
              groupedByPatient.map((g, i) => (
                <div
                  ...
                  className="p-4 flex items-center justify-between border-b border-gray-700/50 last:border-0 cursor-pointer hover:bg-[#242732]/50 active:bg-[#242732]"
                >
                  <div className="flex-1 min-w-0 space-y-1">
                    ...
                    <div className="text-xs text-gray-400 pl-7">{g.patientPhone || 'No phone'}</div>
                    <div className="flex gap-4 pl-7 text-xs">
                      <span className="text-gray-300">Total: ${g.totalFromRegistration.toFixed(2)}</span>
                      <span className="text-green-400 font-medium">Paid: ${g.totalPaid.toFixed(2)}</span>
                      <span className="text-gray-400">Balance: ${g.totalBalance.toFixed(2)}</span>
                    </div>
                  </div>
                  <div className="flex gap-2 ml-2" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button onClick={() => setHistoryPatient({ id: g.patientId, name: g.patientName, phone: g.patientPhone })} className="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-700/40 text-gray-400" title="View history">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    </button>
                  </div>
                </div>
