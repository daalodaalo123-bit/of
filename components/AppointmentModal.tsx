...
export default function AppointmentModal({ appointment, onClose, onSave }: AppointmentModalProps) {
  ...
  const inputClass = "w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-200 bg-accent text-foreground placeholder-gray-500 text-sm sm:text-base"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mb-4 sm:mb-6">
            <div className="relative">
              ...
              {showPatientList && filteredPatients.length > 0 && (
                <div ref={patientListRef} className="absolute z-50 w-full mt-2 bg-accent border border-border rounded-xl shadow-xl max-h-60 overflow-y-auto">
                  {filteredPatients.map((patient) => (
                    <button
                      key={patient.id}
                      type="button"
                      onClick={() => handlePatientSelect(patient)}
                      className="w-full px-4 py-3 text-left hover:bg-blue-900/30 transition-colors border-b border-border last:border-b-0 first:rounded-t-xl last:rounded-b-xl"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-foreground truncate mb-0.5">{patient.name}</div>
                          <div className="text-sm text-muted truncate">{patient.email || '-'}</div>
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">Time Slot Display *</label>
