...
export default function AppointmentModal({ appointment, onClose, onSave }: AppointmentModalProps) {
  ...
  const handlePatientSelect = (patient: Patient) => {
    setFormData({ ...formData, patientName: patient.name, patientId: patient.id })
  ...
  const inputClass = "w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-200 bg-[#242732] text-gray-200 placeholder-gray-500 text-sm sm:text-base"
      ...
      <div className="bg-[#1a1d27] rounded-2xl shadow-2xl max-w-2xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-y-auto border border-gray-700/50" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 lg:p-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mb-4 sm:mb-6">
            <div className="relative">
              ...
              <input
                ...
                onFocus={() => { if (patientSearchTerm.length > 0 && filteredPatients.length > 0) setShowPatientList(true) }}
                className={inputClass}
                placeholder="Enter patient name"
              />
              {showPatientList && filteredPatients.length > 0 && (
                <div ref={patientListRef} className="absolute z-50 w-full mt-2 bg-[#242732] border border-gray-700/50 rounded-xl shadow-xl max-h-60 overflow-y-auto">
                  {filteredPatients.map((patient) => (
                    <button
                      key={patient.id}
                      type="button"
                      onClick={() => handlePatientSelect(patient)}
                      className="w-full px-4 py-3 text-left hover:bg-blue-900/30 transition-colors border-b border-gray-700/50 last:border-b-0 first:rounded-t-xl last:rounded-b-xl"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-white truncate mb-0.5">{patient.name}</div>
            ...
            <div>
              ...
              <input type="text" required value={formData.patientId} onChange={(e) => setFormData({ ...formData, patientId: e.target.value })} className={inputClass} placeholder="Enter patient ID" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Date *</label>
              <input type="date" required value={formData.appointmentDate} onChange={(e) => setFormData({ ...formData, appointmentDate: e.target.value })} className={inputClass} />
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Time Slot Display *</label>
              <input type="text" required value={formData.timeSlot} onChange={(e) => setFormData({ ...formData, timeSlot: e.target.value })} className={inputClass} placeholder="e.g., 10:00 AM" />
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-300 mb-2">Treatment Type</label>
              <input type="text" value={formData.treatmentType || ''} onChange={(e) => setFormData({ ...formData, treatmentType: e.target.value })} className={inputClass} placeholder="e.g., General Checkup, Consultation" />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-300 mb-2">Notes</label>
              <textarea rows={3} value={formData.notes || ''} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className={inputClass + " resize-none"} placeholder="Additional notes or instructions..." />
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 sm:pt-6 border-t border-gray-700/50">
            ...
            <button type="submit" className="flex-1 px-5 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all duration-200 font-medium shadow-sm">
