...
export default function Dashboard({ stats, onRefresh }: DashboardProps) {
  ...
  const autoSendReminders = async () => {
    try {
      // This will check if already sent today server-side
      const res = await fetch('/api/reminders/monthly', { method: 'POST' })
      const data = await res.json()
      if (res.ok && !data.alreadySent && data.sentCount > 0) {
        console.log('Automated reminders sent successfully!')
        loadReminderCount() // Update the count to 0
      }
    } catch (e) {}
  }
  ...
  const handleSendReminders = async () => {
    if (!confirm(`Send WhatsApp reminders to ${reminderCount} patients?`)) return
  ...
  const statCards = [
    ...
    { label: 'Revenue', value: `$${(stats.totalRevenue || 0).toLocaleString()}`, icon: '💰', color: 'from-purple-500 to-purple-600', isRevenue: true },
  ]
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      ...
      {/* Reminder Alert - Only shows if there are pending reminders */}
      {reminderCount > 0 && (
        <div className="mb-4 sm:mb-6">
          <div className="bg-gradient-to-r from-emerald-600 to-emerald-800 rounded-2xl p-4 sm:p-6 shadow-lg border border-emerald-500/20 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4 text-center sm:text-left">
              <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center text-2xl animate-pulse">
                🔔
              </div>
              <div>
                <h3 className="text-white font-bold text-lg">{reminderCount} Follow-up Reminders Due</h3>
                <p className="text-white/80 text-sm">Patients who registered/paid 1 month ago need a reminder for tomorrow.</p>
              </div>
            </div>
            <button
              onClick={handleSendReminders}
              disabled={sendingReminders}
              className="w-full sm:w-auto px-6 py-3 bg-white text-emerald-700 rounded-xl font-bold hover:bg-emerald-50 transition-all shadow-md flex items-center justify-center gap-2 whitespace-nowrap"
            >
              {sendingReminders ? (
                <>
                  <svg className="animate-spin h-5 w-5 text-emerald-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                  Sending...
                </>
              ) : (
                <>
      ...
      {analytics?.revenueInsights && (
        <div className="mb-4 sm:mb-6">
          <div className="bg-gradient-to-br from-emerald-600 via-emerald-700 to-teal-800 rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-xl overflow-hidden relative">
              ...
              <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                <div>
                  ...
                  <p className="text-3xl sm:text-4xl font-bold text-foreground">
                    {showRevenueAmounts ? `$${(analytics.revenueInsights.thisMonthRevenue || 0).toLocaleString()}` : '••••••'}
                  </p>
                  <div className="flex items-center gap-2 mt-2">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${
                      (analytics.revenueInsights.changePct || 0) >= 0 ? 'bg-green-400/30 text-white' : 'bg-red-400/30 text-white'
                    }`}>
                      {showRevenueAmounts ? (
                        <>{(analytics.revenueInsights.changePct || 0) >= 0 ? '↑' : '↓'} {Math.abs(analytics.revenueInsights.changePct || 0)}%</>
                      ) : (
                        '•••'
                      )}
                    </span>
                    <span className="text-foreground/70 text-xs">
                      vs last month {showRevenueAmounts ? `($${(analytics.revenueInsights.lastMonthRevenue || 0).toLocaleString()})` : '(••••••)'}
                    </span>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-xl font-semibold text-foreground/90">
                    {showRevenueAmounts ? `$${(analytics.revenueInsights.lastMonthRevenue || 0).toLocaleString()}` : '••••••'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-card rounded-xl sm:rounded-2xl shadow-sm border border-border overflow-hidden">
            ...
            <div className="p-4 lg:p-6 space-y-3">
              {[
                ...
                { key: 'cash', label: 'Cash', color: 'bg-purple-500' },
