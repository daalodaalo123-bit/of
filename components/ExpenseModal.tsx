...
export default function ExpenseModal({ expense, onClose, onSave }: ExpenseModalProps) {
  ...
  useEffect(() => {
    if (expense) {
      ...
      setFormData({ amount: 0, category: 'other', description: '', expenseDate: new Date().toISOString().slice(0, 10) })
    }
  }, [expense])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { ...formData, id: expense?.id || `expense-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, amount: Number(formData.amount) || 0, expenseDate: formData.expenseDate }
