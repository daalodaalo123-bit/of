...
export async function GET() {
  try {
    ...
    // Outstanding balances (Patient.totalDue - total paid)
    const allPatientsForBalances = await Patient.find({ totalDue: { $gt: 0 } }).lean()
    const patientOutstanding = allPatientsForBalances.map((patient: any) => {
      const patientPayments = allPayments.filter((p: any) => p.patientId === patient.id)
      const totalPaid = patientPayments.reduce((sum: number, p: any) => sum + (p.amountPaid || 0), 0)
      const balance = Math.max(0, (patient.totalDue || 0) - totalPaid)
      return {
        patientName: patient.name,
        remainingBalance: balance,
        totalAmount: patient.totalDue || 0,
      }
    })
    .filter((p: any) => p.remainingBalance > 0)
    .sort((a, b) => b.remainingBalance - a.remainingBalance)

    const topOutstanding = patientOutstanding.slice(0, 10)
    const totalOutstanding = patientOutstanding.reduce((s: number, p: any) => s + p.remainingBalance, 0)
    ...
    return NextResponse.json({
      ...
      outstandingBalances: {
        ...
        count: patientOutstanding.length,
        topPatients: topOutstanding,
      },
