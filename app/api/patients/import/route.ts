  ...
  try {
    ...
    for (const row of data as any[]) {
      try {
        ...
        const patientData = {
          id: row.ID || row.id || `patient-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
          ...
          doctorId: row['Doctor ID'] || row.doctorId || null,
          doctorName: row['Doctor Name'] || row.doctorName || null,
          totalDue: Number(row['Total Due'] || row.totalDue) || 0,
          createdAt: row['Created At'] || row.createdAt || new Date(),
          updatedAt: new Date(),
        }
